...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        //Com restrição de integralidade
        for(int i = 0; i < qtdOrigens; i++)
            Y1.add(IloNumVar(env, 0, IloInfinity, ILOINT));
            
        for(int j = 0; j < qtdDestinos; j++)
            Y2.add(IloNumVar(env, 0, IloInfinity, ILOINT)); 
        ...
        for(int i = 0; i < qtdOrigens; i++){
            for (int j = 0; j < qtdDestinos; ++j) {
                ...
                //modelo.add(IloRange(env, -IloInfinity, exp2, 0));
